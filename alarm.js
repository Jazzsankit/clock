...
const alarmText = (time) => `<h3>Alarm set at time ${time}</h3>`;
...
const handleSubmit = (event) => {
  ...
  tempAlarmString = getTimeString({
    hours: hour.value,
    seconds: sec.value,
    minutes: min.value,
    zone: zone.value
  });
  if(alarmString.length==3){
    alert("more than 3 alarm");
    handleClear();
  return;
  }
  ...
  alarmString.push(tempAlarmString);
  alarmTextContainer.innerHTML += alarmText(tempAlarmString);
};
